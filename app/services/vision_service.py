...
def analyser_visuel_acte(pdf_bytes: bytes) -> dict:
    ...
    prompt_visuel = (
        ...
        "IMPORTANT sur la forme des tampons : sur ces documents, un tampon peut "
        "être soit un cachet encré classique (rond/rectangulaire), SOIT une "
        "zone délimitée par un cadre en pointillés/tirets contenant le nom de "
        "la direction (ex: un encadré en pointillés avec le texte 'DGFP' "
        "dedans). Ces deux formes comptent comme un tampon PRÉSENT — ne les "
        "confonds pas avec un espace totalement vide ou un simple filigrane "
        "sans aucun texte de direction dedans.\n\n"
        "Cherche précisément sur l'ensemble des pages :\n"
        "- le numéro de l'acte en haut de la première page (numero_acte_haut)\n"
        "- les tampons DGFP, DIRSOLDE, DPB, CF, DP : réponds true dès que tu "
        "identifies un cachet OU un encadré (même en pointillés) portant ce "
        "nom de direction, peu importe le style graphique\n"
        "- la signature manuscrite ET le cachet du Ministre, généralement en "
        "bas de la dernière page (signature_cachet_ministre_bas)\n"
        "Réponds false uniquement si la zone est totalement vide (aucun texte "
        "de direction visible dedans) ou si l'élément n'apparaît nulle part. "
        "Dans 'details', précise sur quelle page tu as trouvé chaque élément "
        "et sous quelle forme (cachet plein ou encadré en pointillés)."
    )
