...
⚠️ SOLUTION TEMPORAIRE — l'objectif final du projet est une solution 100%
open source auto-hébergée (voir la tentative Moondream2 via llama-server,
mise en pause faute de temps/qualité suffisante sur ce modèle précis :
trop lent — ~60s rien que pour encoder l'image — et peu fiable pour suivre
une instruction JSON à 8 champs). Les fichiers modèles téléchargés restent
sur le serveur (~/models/moondream2/) pour reprendre ce travail.
...
def analyser_visuel_acte(pdf_bytes: bytes) -> dict:
    ...
    prompt_visuel = (
        ...
        "PAS un espace vide ni un simple filigrane à ignorer. Réponds true dès "
        "que tu distingues ces lettres en pointillés formant le nom d'une "
        "direction, peu importe leur netteté.\n\n"
